...
public class Stage5_Boss : MonoBehaviour
{
    float baseDir = 0;
    ...
    public IEnumerator StltAttack()
    {
        ...
        {
            if (null != child.GetComponent<SatelliteMove>())
            {
                child.GetComponent<SatelliteMove>().SetIsSAMove(true);
            }
        }
        ...
        while (true)
        {
            ...
            foreach (Transform child in this.transform)
            {
                yield return new WaitForSeconds(0.01f);
                if (null != child.GetComponent<SatelliteMove>())
                    if (!child.GetComponent<SatelliteMove>().isSAMoved)
                        goto A;
            }
            break;
        }
        ...
        foreach (Transform child in this.transform)
        {
            if (null != child.GetComponent<SatelliteMove>())
            {
                child.GetComponent<SatelliteMove>().SetIsSAttack(true);
            }
        }
    }
